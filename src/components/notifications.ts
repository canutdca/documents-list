...
export class Notifications extends HTMLElement {
  ...
  private render() {
    this.innerHTML = `
      <div role="status" aria-live="polite" aria-label="New documents created alert" class="fixed top-0 left-1/2 -translate-x-1/2 z-50 p-4 ${this.count === 0 ? 'hidden' : ''}">
        <div class="bg-white rounded-full px-3 py-1.5 text-xs text-gray-700 animate-fade-in flex items-center gap-2">
          <div class="relative">
            ${bellIcon}
            <span role="status" aria-live="polite" aria-label="Counter of new documents created" class="absolute -top-1 -right-1 bg-blue-500 text-white text-[8px] font-bold rounded-full w-3 h-3 flex items-center justify-center">
              ${this.count}
            </span>
          </div>
